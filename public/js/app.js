...
document.addEventListener('DOMContentLoaded', () => {
      ...
      if (window.__adhelloBulkOutreachInflight) {
        if (typeof window.showBulkActionConfirmation === 'function') {
          window.showBulkActionConfirmation(
            'A bulk send is already running — check the notification bell.',
            'error',
          );
        }
        return true;
        ...
        try {
          for (let i = 0; i < leadKeys.length; i += 1) {
            ...
            if (window.agencyOsBulkOutreach && typeof window.agencyOsBulkOutreach.progress === 'function') {
              window.agencyOsBulkOutreach.progress({ done: i + 1, total: n, ok, failed, skipped });
            }
            if (typeof window.showBulkActionConfirmation === 'function') {
              window.showBulkActionConfirmation(
            ...
            try {
              if (emailMode) {
                ...
                const rowEmail = (function findEmail() {
                  try {
                    const rows = document.querySelectorAll('tr[data-lead-key]');
                    for (let r = 0; r < rows.length; r += 1) {
                      const k = String(rows[r].getAttribute('data-lead-key') || '').trim();
                      if (k !== leadKey && k !== String(leadKey).replace(/^lead:/, '')) continue;
                      const em = String(rows[r].getAttribute('data-email') || '').trim();
              ...
              console.warn('Early bulk send failed for', leadKey, msg);
            }
          }
          const summaryParts = [
            `${emailMode ? 'Email' : 'SMS'}: ${ok} sent`,
            skipped ? `${skipped} skipped` : '',
            failed ? `${failed} failed` : '',
          ].filter(Boolean);
          const summary = summaryParts.join(' · ');
          if (window.agencyOsBulkOutreach && typeof window.agencyOsBulkOutreach.finish === 'function') {
            window.agencyOsBulkOutreach.finish({
              channel: emailMode ? 'email' : 'sms',
              ok,
              failed,
              skipped,
              lastError,
            });
          } else if (typeof window.agencyOsPushBellNotification === 'function') {
            window.agencyOsPushBellNotification({
              headline: failed ? 'Send finished with errors' : 'Send complete',
              body: lastError ? `${summary} — ${lastError.slice(0, 160)}` : summary,
  ...
  function syncHeaderPhoneRow(row, snapPhone) {
    ...
    const phoneRow = document.getElementById('headerPhoneRow');
    ...
    if (phoneRow) phoneRow.classList.toggle('hidden', !phone);
    if (starBtn) {
  ...
  function syncHeaderWebsiteRow(row) {
    ...
    const websiteRow = document.getElementById('headerWebsiteRow');
    if (!view) return;
    const website = readPipelineRowDisplayWebsite(row);
    const href = website ? normalizeWebsiteHref(website) : '';
    if (href) {
      view.href = href;
    ...
    if (websiteRow) websiteRow.classList.toggle('hidden', !href);
  }

  function syncHeaderEmailRow(row) {
    const view = getLeadPanelEl('mobilePanelHeaderEmail');
    const emailRow = document.getElementById('headerEmailRow');
    if (!view) return;
    const emailRaw = readPipelineRowDisplayEmail(row);
    const email =
      emailRaw && emailRaw.includes('@') && !isEmptyLeadField(emailRaw) ? emailRaw : '';
    if (email) {
  ...
  function renderLeadPanelContactLinksHtml(row) {
    if (!row || !row.dataset) return '';
    const parts = [];
    const websiteHref = normalizeWebsiteHref(readPipelineRowDisplayWebsite(row));
    const phone = readPipelineRowDisplayPhone(row);
    const emailRaw = readPipelineRowDisplayEmail(row);
    const email =
      emailRaw && emailRaw.includes('@') && !isEmptyLeadField(emailRaw) ? emailRaw : '';
  ...
  async function sendEmailToLeadKey(leadKey, subject, body, opts) {
    ...
    if (toEmail && toEmail.includes('@')) {
      payload.to = toEmail;
      payload.saveToLead = opts.saveToLead !== false;
    }
  ...
  function resolveRowEmailForLeadKey(leadKey) {
    const want = String(leadKey || '').trim();
    if (!want) return '';
    const rows = document.querySelectorAll('tr.result-row[data-lead-key], tr[data-lead-key]');
    for (let i = 0; i < rows.length; i += 1) {
      const row = rows[i];
      const key = String(row.getAttribute('data-lead-key') || row.dataset.leadKey || '').trim();
      if (key !== want && key !== want.replace(/^lead:/, '') && `lead:${key}` !== want) continue;
      if (typeof coalesceRowDatasetFromContacts === 'function') coalesceRowDatasetFromContacts(row);
      if (typeof hydrateRowDatasetFromTableDom === 'function') hydrateRowDatasetFromTableDom(row);
      const email = String(row.getAttribute('data-email') || row.dataset.email || '').trim();
  ...
  function notifyBulkOutreachProgress(message, variant) {
    if (typeof showBulkSaveFeedback === 'function') {
      showBulkSaveFeedback(message, variant || 'loading');
    } else if (typeof window.showAppToast === 'function') {
      window.showAppToast(message, { variant: variant === 'error' ? 'error' : 'info', duration: 3200 });
    }
  }

  function finishBulkOutreachBell(result, channel) {
    const api = window.agencyOsBulkOutreach;
    ...
    if (typeof window.agencyOsPushBellNotification === 'function') {
      const parts = [`${channel === 'sms' ? 'SMS' : 'Email'}: ${result.ok || 0} sent`];
      if (result.skipped) parts.push(`${result.skipped} skipped`);
      if (result.failed) parts.push(`${result.failed} failed`);
      window.agencyOsPushBellNotification({
        headline: result.failed ? 'Send finished with errors' : 'Send complete',
        body: result.lastError ? `${parts.join(' · ')} — ${String(result.lastError).slice(0, 160)}` : parts.join(' · '),
        href: '/prospecting?tab=pipeline',
        linkLabel: 'Open pipeline →',
        desktop: true,
      });
  ...
  async function sendBulkPersonalizedEmail(emailKeys, scriptText, subject, onProgress) {
    ...
    for (let i = 0; i < emailKeys.length; i += 1) {
      ...
      try {
        ...
        const toFromRow = resolveRowEmailForLeadKey(leadKey);
        ...
        if (
          /no email|email is required|missing email|not found|recipient email|no recipient email|on file in Agency OS/i.test(
            msg,
          )
        ) {
          skipped += 1;
  ...
  }
  {
    ...
    async function handleSmsScriptSend() {
      ...
      if (activeMode === 'bulk-email' && activeKeys.length) {
        ...
        notifyBulkOutreachProgress(`Sending email in background (0/${n})…`, 'loading');
        closeSmsModal();
        window.__adhelloBulkOutreachInflight = true;
        if (window.agencyOsBulkOutreach && typeof window.agencyOsBulkOutreach.start === 'function') {
        ...
        void (async function runBulkEmailInBackground() {
          try {
            const result = await sendBulkPersonalizedEmail(keys, script, subject, (done, total) => {
              notifyBulkOutreachProgress(`Personalizing & sending email ${done}/${total}…`, 'loading');
              if (window.agencyOsBulkOutreach && typeof window.agencyOsBulkOutreach.progress === 'function') {
            ...
            const finished = finishBulkOutreachBell(result, 'email');
            const msg =
              (finished && finished.summary) ||
              [
                `Email: ${result.ok} sent`,
            ...
            finishBulkOutreachBell(
              { ok: 0, failed: 1, skipped: 0, lastError: (err && err.message) || 'Bulk email failed.' },
              'email',
            );
            notifyBulkOutreachProgress(err.message || 'Bulk email failed.', 'error');
          } finally {
            window.__adhelloBulkOutreachInflight = false;
          }
        })();
      ...
      if (activeMode === 'bulk' && activeKeys.length) {
        ...
        notifyBulkOutreachProgress(`Sending SMS in background (0/${n})…`, 'loading');
        closeSmsModal();
        window.__adhelloBulkOutreachInflight = true;
        if (window.agencyOsBulkOutreach && typeof window.agencyOsBulkOutreach.start === 'function') {
        ...
        void (async function runBulkSmsInBackground() {
          try {
            ...
            finishBulkOutreachBell(
              { ok: 0, failed: 1, skipped: 0, lastError: (err && err.message) || 'Bulk SMS failed.' },
              'sms',
            );
            notifyBulkOutreachProgress(err.message || 'Bulk SMS failed.', 'error');
          } finally {
            window.__adhelloBulkOutreachInflight = false;
          }
        })();
  ...
  async function bulkSaveSelectedLeads(triggerBtn) {
    ...
    if (isProspectPipeline && (folderKey || opportunityPlacement)) {
      ...
      const placeLabel = opportunityPlacement
        ...
        : '';
      showBulkSaveFeedback(
        `Saving ${leadKeys.length} lead${leadKeys.length === 1 ? '' : 's'}${placeLabel ? ` to ${placeLabel}` : folderName ? ` to ${folderName}` : ''}…`,
        'loading',
      );
  ...
  window.renderLeadsReviewsInnerHtml = renderLeadsReviewsInnerHtml;
  ...
  const BULK_REVIEWS_FETCH_TIMEOUT_MS = 75000;
  const BULK_REVIEWS_LEAD_TIMEOUT_MS = 90000;

  function syncReviewsCellAfterRefresh(row) {
    if (!row) return;
    const layout = getBulkEnhanceLayout(row);
    if (layout && layout.kind === 'leads' && layout.reviews) {
      layout.reviews.innerHTML = renderLeadsReviewsInnerHtml(
        row.dataset.rating,
        row.dataset.reviews,
        row,
      );
  ...
  async function runRefreshReviewsApiForLeadKey(leadKey) {
    const key = String(leadKey || '').trim();
    ...
    const run = async () => {
      ...
      if (typeof window.__fetchJsonWithTimeout === 'function') {
        post = await window.__fetchJsonWithTimeout(
          ...
          BULK_REVIEWS_FETCH_TIMEOUT_MS,
        );
      } else {
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), BULK_REVIEWS_FETCH_TIMEOUT_MS);
        try {
    ...
    if (typeof window.__withBulkEnhanceLeadTimeout === 'function') {
      return window.__withBulkEnhanceLeadTimeout(run(), BULK_REVIEWS_LEAD_TIMEOUT_MS);
    }
    return Promise.race([
      run(),
      new Promise((_, reject) => {
        setTimeout(() => {
          reject(new Error('Reviews refresh timed out after 90s for this lead.'));
        }, BULK_REVIEWS_LEAD_TIMEOUT_MS);
      }),
    ]);
  }

  async function runBulkReviewsRefreshSelectedLeads() {
    const checkedBoxes = document.querySelectorAll('.row-checkbox:checked, .lead-checkbox:checked');
    if (checkedBoxes.length === 0) {
      notifyBulkEnhanceIdle('Select one or more saved leads to refresh reviews.');
      return;
    }
    ...
    if (!leadsToProcess.length) {
      notifyBulkEnhanceIdle(
        'Save leads to your pipeline first — review refresh needs saved lead keys.',
        'warning',
      );
      return;
    }
    if (selectedRows.length > 25) {
      notifyBulkEnhanceIdle('Review refresh limited to 25 leads per batch.', 'info');
    }
    ...
    if (typeof window.__showBulkBarFeedbackEarly === 'function') {
      window.__showBulkBarFeedbackEarly(
        `Refreshing reviews for ${leadsToProcess.length} lead${leadsToProcess.length === 1 ? '' : 's'}…`,
        'loading',
      );
    }
    ...
    try {
      ...
      for (let i = 0; i < leadsToProcess.length; i += 1) {
        ...
        if (typeof window.__showBulkBarFeedbackEarly === 'function') {
          window.__showBulkBarFeedbackEarly(
            `Refreshing Google reviews ${i + 1}/${total}…`,
            'loading',
          );
        }
        const layout = getBulkEnhanceLayout(row);
        if (layout && layout.reviews) {
          layout.reviews.innerHTML =
            '<span class="text-[9px] font-bold text-amber-400 uppercase tracking-widest animate-pulse">Updating…</span>';
        }
        try {
          const result = await runRefreshReviewsApiForLeadKey(key);
          if (result.error) lastError = String(result.error);
          const d = result.lead;
          const ok = !!result.success && !!d;
          if (ok) successCount += 1;
          if (d) {
            applyEnrichDataToRowDataset(row, d, result);
            syncReviewsCellAfterRefresh(row);
          } else if (layout && layout.reviews) {
            syncReviewsCellAfterRefresh(row);
          }
        } catch (err) {
    ...
    if (typeof window.__showBulkBarFeedbackEarly === 'function' && attemptedCount > 0) {
      window.__showBulkBarFeedbackEarly(
        successCount > 0
          ...
          : lastError || 'No Google reviews found for the selected lead(s).',
    ...
    if (successCount === 0 && attemptedCount > 0) {
      ...
      notifyBulkEnhanceIdle(
        `Refreshed Google reviews & freshness on ${successCount} lead${successCount !== 1 ? 's' : ''}.`,
        'ok',
      );
    }
  ...
  /** Progress on #bulkSaveFeedback; toast only for terminal / validation (not every N/M tick). */
  function showBulkVerifyFeedback(message, variant, opts) {
    const msg = String(message || '').trim();
    const options = opts && typeof opts === 'object' ? opts : {};
    const el = document.getElementById('bulkSaveFeedback');
    if (el) {
      ...
      else if (variant === 'info' || variant === 'warning') el.classList.add('text-sky-200');
    ...
    const shouldToast =
      ...
      (options.toast !== false &&
        ...
        (variant === 'error' ||
          ...
          variant === 'warning' ||
          variant === 'info'));
    if (shouldToast && typeof window.showAppToast === 'function') {
      const toastVariant =
        variant === 'error'
          ? 'error'
          : variant === 'success' || variant === 'ok'
            ? 'success'
            : 'info';
      window.showAppToast(msg, {
        variant: toastVariant,
        duration: variant === 'error' ? 9000 : 5200,
      });
    } else if (shouldToast && typeof window.showProspectToast === 'function') {
      window.showProspectToast(msg);
    }
  }

  async function ensureLeadKeyQuietForPhoneVerify(row) {
    if (!row) return '';
    let key =
      typeof syncRowLeadKeyFromSavedMap === 'function' ? syncRowLeadKeyFromSavedMap(row) : '';
    if (!key && typeof findInitialSavedLeadRecord === 'function') {
      const embedded = findInitialSavedLeadRecord(row);
      const embeddedKey = embedded && embedded.key ? String(embedded.key).trim() : '';
      if (embeddedKey) {
    ...
    try {
      ...
      if (!ok) return '';
      return typeof syncRowLeadKeyFromSavedMap === 'function'
        ? syncRowLeadKeyFromSavedMap(row)
        : String(row.dataset.leadKey || '').trim();
    } catch (_) {
      return '';
    }
  }
  ...
  async function runBulkVerifyPhoneLineSelectedLeads() {
    ...
    const checkedBoxes = document.querySelectorAll('.row-checkbox:checked, .lead-checkbox:checked');
    if (checkedBoxes.length === 0) {
      showBulkVerifyFeedback('Select one or more leads with phone numbers to verify.', 'error');
      return;
    }
    ...
    selectedRows.forEach((r) => {
      if (typeof syncRowLeadKeyFromSavedMap === 'function') syncRowLeadKeyFromSavedMap(r);
    });

    const withPhone = selectedRows.filter((r) => rowHasUsablePhoneForVerify(r));
    if (!withPhone.length) {
      showBulkVerifyFeedback(
    ...
    const setVerifyBusyLabel = (label) => {
      const html = `<span class="text-[10px] font-black uppercase tracking-widest animate-pulse">${label}</span>`;
    ...
    const restoreVerifyBtns = (delayMs) => {
      window.setTimeout(() => {
    ...
    setVerifyBusyLabel('Verifying…');

    const batchCandidates = withPhone.slice(0, BULK_VERIFY_PHONES_MAX);
    if (withPhone.length > BULK_VERIFY_PHONES_MAX) {
      showBulkVerifyFeedback(
        `Verifying first ${BULK_VERIFY_PHONES_MAX} of ${withPhone.length} selected phones…`,
        'info',
      );
    }
    ...
    if (needsSave.length) {
      setVerifyBusyLabel('Saving…');
      showBulkVerifyFeedback(
        `Saving ${needsSave.length} lead${needsSave.length === 1 ? '' : 's'} so phones can be verified…`,
        'loading',
        { toast: false },
      );
      for (let i = 0; i < needsSave.length; i += 1) {
        setVerifyBusyLabel(`Save ${i + 1}/${needsSave.length}`);
        showBulkVerifyFeedback(
          `Saving leads for phone verify ${i + 1}/${needsSave.length}…`,
          'loading',
          { toast: false },
        );
        const key = await ensureLeadKeyQuietForPhoneVerify(needsSave[i]);
        if (!key) saveFailed += 1;
      }
    }

    const leadsToProcess = batchCandidates.filter(
      (r) => String(r.dataset.leadKey || '').trim() && rowHasUsablePhoneForVerify(r),
    );
    ...
    if (!leadsToProcess.length) {
      ...
      const errMsg =
        saveFailed > 0 || needsSave.length
          ? hasFolder
            ? 'Could not save selected leads for phone verify. Try Save in the bar, then Verify phones again.'
            : 'Pick a folder in the bar, save selected leads (or let Verify auto-save), then try again.'
          : 'Select saved leads with a phone number to verify cell vs landline.';
      showBulkVerifyFeedback(errMsg, 'error');
    ...
    try {
      ...
      for (let i = 0; i < leadsToProcess.length; i += 1) {
        ...
        if (!key) continue;
        attemptedCount += 1;
        setVerifyBusyLabel(`Verify ${i + 1}/${total}`);
        showBulkVerifyFeedback(`Verifying cell vs landline ${i + 1}/${total}…`, 'loading', {
          toast: false,
        });
        try {
  ...
  document.addEventListener(
    'click',
    (e) => {
      const btn = e.target && e.target.closest ? e.target.closest('.js-bulk-reviews') : null;
      if (!btn || btn.disabled || btn.classList.contains('loading')) return;
      e.preventDefault();
      e.stopPropagation();
