...
router.all('/telephony/voice/twiml', async (req, res) => {
  try {
    ...
    if (agentFirst) {
      ...
      // and leave the softphone stuck on IN-PROGRESS while dialing the lead into void.
      ...
      const xml = [
        '<?xml version="1.0" encoding="UTF-8"?>',
        '<Response>',
        `<Gather numDigits="1" timeout="18" actionOnEmptyResult="true" action="${xmlEscape(confirmUrl)}">`,
